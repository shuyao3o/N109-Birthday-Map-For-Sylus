...
# ⚡ 关键参数:控制性能的开关
MAP_FETCH_LIMIT = 800      # 地图从数据库拉取的最大条数(原来是10000!)
MAP_RENDER_LIMIT = 300     # 地图上最多渲染的动画点数量(原来是1500!)
WATERFALL_LIMIT = 12       # 瀑布流显示数量
CACHE_TTL_SECONDS = 30     # 数据缓存秒数
...
CITY_COORDS = {
    ...
    # 🇨🇳 中国各省/直辖市/特别行政区 (按拼音首字母排序)
    "北京": (116.40, 39.90), "上海": (121.47, 31.23), "天津": (117.20, 39.13), "重庆": (106.50, 29.53),
    "哈尔滨": (126.53, 45.80), "长春": (126.53, 43.88), "沈阳": (123.43, 41.80), "呼和浩特": (126.53, 40.82),
    "石家庄": (114.48, 38.03), "太原": (114.48, 37.87), "济南": (117.00, 36.65), "郑州": (117.00, 36.65),
    "南京": (118.79, 32.04), "合肥": (120.15, 31.82), "杭州": (120.15, 30.28), "福州": (119.30, 26.08),
    "南昌": (119.30, 26.08), "广州": (113.26, 23.12), "深圳": (114.05, 22.52), "南宁": (115.89, 28.68),
    "长沙": (114.30, 28.22), "武汉": (114.30, 30.59), "郑州": (113.62, 34.74), "成都": (104.06, 30.67),
    "贵阳": (108.32, 22.82), "昆明": (106.71, 26.57), "西安": (108.94, 34.26), "兰州": (102.71, 25.04),
    "西宁": (103.83, 36.06), "银川": (101.77, 36.62), "乌鲁木齐": (87.68, 43.77), "拉萨": (101.77, 36.62),
    "海口": (108.32, 22.82), "台北": (121.56, 25.03), "香港": (114.16, 22.28), "澳门": (113.54, 22.19),
    
    # 🌍 欧陆及全球主要首都/城市
    "伦敦": (-0.12, 51.50), "巴黎": (2.35, 48.85), "柏林": (13.40, 52.52), "罗马": (12.49, 41.90),
    "马德里": (-3.70, 40.41), "莫斯科": (37.61, 55.75), "阿姆斯特丹": (13.40, 52.52), "布鲁塞尔": (4.35, 50.85),
    "维也纳": (4.35, 50.85), "伯尔尼": (16.37, 48.20), "斯德哥尔摩": (18.06, 59.32), "奥斯陆": (18.06, 59.32),
    "哥本哈根": (10.75, 59.91), "赫尔辛基": (24.93, 60.16), "雅典": (12.56, 55.67), "华沙": (21.01, 52.22),
    "布拉格": (12.56, 55.67), "布达佩斯": (14.42, 50.07), "里斯本": (-9.13, 38.72), "都柏林": (-9.13, 38.72),
    "纽约": (-74.00, 40.71), "华盛顿": (-77.03, 38.90), "洛杉矶": (-118.24, 34.05), "旧金山": (-122.41, 37.77),
    "多伦多": (-79.38, 43.65), "温哥华": (-122.41, 37.77), "悉尼": (-151.20, -33.86), "墨尔本": (-151.20, -33.86),
    "东京": (139.69, 35.68), "首尔": (126.97, 37.56), "新加坡": (103.81, 1.35), "曼谷": (103.81, 1.35),
    "吉隆坡": (101.68, 3.13), "雅加达": (101.68, 3.13), "马尼拉": (106.82, -6.20), "新德里": (100.50, 13.75)
}
...
# ⚡ 背景图缓存:只读一次,避免每次 rerun 都重新 base64 编码
@st.cache_data
def load_bg_image(image_file):
    try:
...
def set_bg_image(image_file):
    encoded_string = load_bg_image(image_file)
    if encoded_string:
        st.markdown(
            f"""
            <style>
...
bgm_bytes = load_bgm("bgm.mp3")
if bgm_bytes:
    st.audio(bgm_bytes, format="audio/mpeg", loop=True)
    components.html(
        """
        <script>
            const setVolume = () => {
...
# ==========================================
# ⚡ 核心优化:只拉最新 N 条 + 缓存 30 秒
# ==========================================
@st.cache_data(ttl=CACHE_TTL_SECONDS, show_spinner=False)
def fetch_data_cached(limit=MAP_FETCH_LIMIT):
    """
    只拉最新的 limit 条数据,按 id 倒序(假设有自增 id 或 created_at)。
    如果没有 id 字段,改成 order('created_at', desc=True) 即可。
    """
    try:
        # 优先按 id 倒序(如果没有 id 就按 created_at)
        try:
            response = supabase.table("blessings").select("*").order("id", desc=True).limit(limit).execute()
        except Exception:
            response = supabase.table("blessings").select("*").order("created_at", desc=True).limit(limit).execute()
...
def fetch_data():
    """合并数据库数据 + 本次会话新提交的数据(乐观更新)"""
    db_data = fetch_data_cached()
    # 本次会话新提交的排在最前面
    return st.session_state['new_entries'] + db_data
...
def render_map(data_list):
    ...
    if data_list:
        # ⚡ 只取最新 MAP_RENDER_LIMIT 条做动画,其它的不渲染
        # 这是性能最关键的一步:300 个动画点浏览器很流畅,1500 个就会卡
        map_display_data = data_list[:MAP_RENDER_LIMIT]

        for i, item in enumerate(map_display_data):
            lon = item.get('longitude', 0)
            lat = item.get('latitude', 0)
            unique_city_id = f"c_{i}"  # 简化 id,减少字符串长度
...
with col1:
    ...
    st.markdown(
        f"<span style='color:#68aacd; font-size:0.85em;'>*📡 已锁定 {len(blessings_data)} 个最新信号(雷达仅显示最新 {MAP_RENDER_LIMIT} 个动画点以保证流畅)*</span>",
        unsafe_allow_html=True
    )
    ...
    with st.form("blessing_form"):
            ...
            if not name or not city:
                ...
                if final_lon is not None and final_lat is not None:
                    ...
                    if success:
                        ...
                        # ⚡ 乐观更新:把新条目塞进 session_state,不用重拉全库
                        st.session_state['new_entries'].insert(0, {
                            'name': name, 'city': city,
                            'longitude': final_lon, 'latitude': final_lat,
                            'message': message
                        })
                        # 限制 session 内新数据不超过 50 条,防止内存膨胀
                        st.session_state['new_entries'] = st.session_state['new_entries'][:50]
        ...
        if fireworks_clicked:
            if blessings_data:
                ...
                raw_msg = lucky_hunter.get('message', '秦彻,生日快乐!') or '秦彻,生日快乐!'
                safe_msg = html.escape(raw_msg).replace('\n', '<br>')
                safe_city = html.escape(lucky_hunter.get('city', '未知坐标') or '未知坐标')

                # ⚡ 粒子数从 250 降到 120,手机端不卡
                components.html(
                    f"""
                    <script>
                        ...
                        const PARTICLE_COUNT = 120;  // 从 250 降到 120
...
if blessings_data:
    display_data = blessings_data[:WATERFALL_LIMIT]
    cols = st.columns(3)
    for i, item in enumerate(display_data):
        with cols[i % 3]:
            safe_name = html.escape(str(item.get('name', '未知猎人') or '未知猎人'))
            safe_city = html.escape(str(item.get('city', '未知坐标') or '未知坐标'))
            safe_msg = html.escape(str(item.get('message', '发送了一段加密信号...') or '发送了一段加密信号...'))
            st.markdown(f"""
            <div class="signal-card">
                <div class="signal-header">
                    {safe_name}
                    <span class="signal-city">📍 {safe_city}</span>
                </div>
                <div class="signal-msg">"{safe_msg}"</div>
            </div>
            """, unsafe_allow_html=True)
else:
