...
CITY_COORDS = {
    ...
    "温哥华": (-79.38, 43.65), "vancouver": (-123.12, 49.28),
...
# 🔥 核心性能优化：加入 TTL=60 秒的数据缓存。避免每次点击都重复查询 1 万条数据
@st.cache_data(ttl=60)
def fetch_data():
    try:
        response = supabase.table("blessings").select("*").limit(2000).execute() # 考虑到渲染最大所需量，限制为2000也能提速
        data = response.data
        ...
        if submitted:
            if not name or not city:
                ...
                if final_lon is not None and final_lat is not None:
                    ...
                    if success:
                        fetch_data.clear() # 🔥 优化：写入新数据后清除缓存，保证立即看到自己点亮的坐标
